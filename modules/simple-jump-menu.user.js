...
function buildJumpContainer(jq, mount) {
  let modules = jq(".context_module");
  let innerHTML = "";
  for (let i = 0; i < Math.max(0, modules.length - 1); i++) {
    innerHTML += getJumpTile(jq, modules[i]);
  }
  mount.append("<div id='lcsd-jump-toc'><label for='"+checkboxID+"'><input type='checkbox' id='"+checkboxID+"'> Table of Contents</label></div><div id='lcsd-jump-container'>" + innerHTML + "</div>");
}
